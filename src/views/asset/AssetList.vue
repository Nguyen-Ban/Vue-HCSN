<template>
  <div class="page-container">
    ...
    <MsTable :columns="tableColumns" :data="paginatedAssetData">
        ...
        <template #hmKhLuyKe="{ value }">
            <div style="text-align: right;">{{ formatMoney(value) }}</div>
        </template>

        <template #giaTriConLai="{ value }">
             <div style="text-align: right;">{{ formatMoney(value) }}</div>
        </template>
        ...
        <template #footer>
            <table style="width: 100%; border-collapse: collapse; table-layout: fixed;">
                ...
                <tfoot>
                    <tr>
                        <td colspan="6" style="text-align: left; padding: 10px 16px; border-bottom: 1px solid #e0e0e0; vertical-align: middle;">
                            <div style="display: flex; align-items: center; gap: 12px;">
                                ...
                                <span style="display: inline-flex; align-items: center; gap: 4px;">
                                    <button class="ms-page-btn" :disabled="!canPrev" @click="goToPage(currentPage - 1)"><div class="icon icon-arrow-pag-left"></div></button>
                                    ...
                                    <button class="ms-page-btn" :disabled="!canNext" @click="goToPage(currentPage + 1)"><div class="icon icon-arrow-pag-right"></div></button>
                                </span>
                            </div>
                        </td>
                        <td style="text-align: right; padding: 10px 16px; border-bottom: 1px solid #e0e0e0;">{{ totals.quantity }}</td>
                        <td style="text-align: right; padding: 10px 16px; border-bottom: 1px solid #e0e0e0;">{{ formatMoney(totals.nguyenGia) }}</td>
                        <td style="text-align: right; padding: 10px 16px; border-bottom: 1px solid #e0e0e0;">{{ formatMoney(totals.hmKhLuyKe) }}</td>
                        <td style="text-align: right; padding: 10px 16px; border-bottom: 1px solid #e0e0e0;">{{ formatMoney(totals.giaTriConLai) }}</td>
                        <td style="text-align: right; padding: 10px 16px; border-bottom: 1px solid #e0e0e0;"></td>
                        <td style="text-align: right; padding: 10px 16px; border-bottom: 1px solid #e0e0e0;"></td>
                        <td style="text-align: right; padding: 10px 16px; border-bottom: 1px solid #e0e0e0;">{{ formatMoney(totals.giaTriHaoMonNam) }}</td>
...
const tableColumns = [
    ...
    { key: 'nguyenGia', title: 'Nguyên giá', width: '180px', align: 'right' },
    { key: 'hmKhLuyKe', title: 'HM/KH lũy kế', width: '180px', align: 'right' },
    { key: 'giaTriConLai', title: 'Giá trị còn lại', width: '140px', align: 'right' },
    { key: 'namSuDung', title: 'Năm sử dụng', width: '100px', align: 'right' },
    { key: 'tyLeHaoMon', title: 'Tỷ lệ hao mòn (%)', width: '120px', align: 'right' },
    { key: 'giaTriHaoMonNam', title: 'Giá trị hao mòn năm', width: '150px', align: 'right' },
];
...
const totals = computed(() => {
    return paginatedAssetData.value.reduce((acc, item) => {
        ...
        return acc;
    }, { quantity: 0, nguyenGia: 0, hmKhLuyKe: 0, giaTriConLai: 0, tyLeHaoMon: 0, giaTriHaoMonNam: 0 });
});
...
:deep(.ms-input-wrapper), :deep(.ms-combo-wrapper) {
